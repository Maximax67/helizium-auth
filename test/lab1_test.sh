...
# 1. Перевірка чи запущено контейнер з назвою 'auth-service'
if [ $(docker ps -q -f name=auth-service | wc -l) -eq 0 ]; then
  echo "❌ Error: Container named 'auth-service' not found."
  exit 1
fi
